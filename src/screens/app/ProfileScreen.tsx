...
const ReferFriendIcon = ({ color }: { color: string }) => (
  ...
  const menuList = [
    {
      ...
      renderIcon: () => <ReferFriendIcon color={colors.primary} />,
      onPress: async () => {
        try {
          await Share.share({
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.white }]}>
      <View style={[styles.header, { borderBottomColor: colors.lightGray }]}>
        <TouchableOpacity
          ...
          style={[styles.backButton, { backgroundColor: isDarkMode ? "#2C2C2E" : "#FFFFFF" }]}
        >
          <Ionicons name="chevron-back" size={22} color={colors.primary} />
        </TouchableOpacity>
        <Text style={[styles.title, { color: colors.black }]}>{t("settings_title")}</Text>
      ...
      >
        {user && (
          <View style={styles.profileSection}>
            ...
            <View style={{ alignItems: "center" }}>
              <Text style={[styles.nameText, { color: colors.black }]}>{user?.name || "User Name"}</Text>
              <Text style={[styles.emailText, { color: colors.gray }]}>{user?.email}</Text>
            </View>
          </View>
        )}
        ...
        <View style={styles.menuSection}>
            ...
            {menuList.map((menuItem, index) => (
              ...
              >
                <View style={styles.menuRowLeft}>
                  ...
                  <Text style={[styles.menuItemText, { color: colors.black }]}>{menuItem.title}</Text>
                </View>
          ...
          <View style={styles.secondaryMenuBlock}>
            {secondaryMenuList.map((menuItem, index) => (
              <View key={index}>
                {index > 0 && (
                  <LinearGradient
                    colors={isDarkMode ? ["rgba(255,255,255,0)", "rgba(255,255,255,0.08)", "rgba(255,255,255,0)"] : ["rgba(17,17,17,0)", "rgba(180,186,197,0.4)", "rgba(17,17,17,0)"]}
                    start={{ x: 0, y: 0.5 }}
                    end={{ x: 1, y: 0.5 }}
                    style={{
                      width: "100%",
                ...
                >
                  <Text style={[styles.secondaryMenuText, { color: colors.black }]}>{menuItem.title}</Text>
                  <Ionicons name="chevron-forward" size={18} color="#AEAEB2" />
                </TouchableOpacity>
              </View>
            ))}
          ...
          <TouchableOpacity
            onPress={logOut}
            activeOpacity={0.8}
            style={[styles.logoutButton, { backgroundColor: isDarkMode ? "rgba(255, 255, 255, 0.06)" : "#F2F2F7" }]}
          >
            <MaterialIcons name="logout" size={20} color={isDarkMode ? "#E5E5EA" : "#8E8E93"} />
            <Text style={[styles.logoutText, { color: isDarkMode ? "#E5E5EA" : "#8E8E93" }]}>{t("logout")}</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
      ...
      <Modal visible={langModalVisible} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={[styles.bottomSheetContainer, { backgroundColor: colors.white }]}>
            <View style={styles.bsHeader}>
              <View>
                <Text style={[styles.bsTitle, { color: colors.black }]}>{t("language")}</Text>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 30 }}>
              {availableLanguages.map((item) => {
                ...
                return (
                  <TouchableOpacity
                    key={item.code}
                    style={[styles.langItemRow, { backgroundColor: isDarkMode ? "#2C2C2E" : "#F2F2F7" }]}
                    onPress={async () => {
                  ...
                  >
                    <View style={{ flexDirection: "row", alignItems: "center" }}>
                      <View style={[styles.langPrefixContainer, { backgroundColor: colors.lightPrimary }]}>
                        <Text style={[styles.langPrefixText, { color: colors.primary }]}>
                          {item.code.toUpperCase()}
                        </Text>
                      </View>
                      <View style={{ marginLeft: 12 }}>
                        <Text style={[styles.langNameText, { color: colors.black }]}>{item.name}</Text>
